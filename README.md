...
### 2025.05.12
- **会议论文底部显示所属会议**：迟来的更新，每篇会议论文下方，显示所属会议名称。

### 2025.04.01
- **初步支持bioRxiv论文源**：逐步尝试接入arXiv以外的预印本论文源，目前已经接入了bioRxiv，欢迎测试；数据方面可能仍有不完善之处，如发现不合理的地方，欢迎及时报告。

### 2025.03.18
- **优化Markdown渲染效果**：Kimi API同步到最新版后，[Kimi]的输出内容更丰富（更丰富的公式、表格等），为此对相应的前端渲染引擎做了调整，优化渲染效果。

### 2025.02.10
- **切换最新版Kimi API**：将Kimi API同步到最新版（当前效果最好版本），理论上[Kimi]效果会有明显提升，大致表现为输出更加详细准确、格式化更佳、对公式支持更好等。

### 2025.01.15
- **页面内搜索增加“与”逻辑**：页面内搜索，Include关键词，原本设计是每行一个关键词，只要命中一个就算匹配（即OR逻辑），现可选切换AND逻辑，即必须同时命中所有关键词才算匹配。

### 2024.11.25
- **arXiv类别差集功能**：arXiv类别同时支持并集和差集，比如`https://papers.cool/arxiv/cs.AI+cs.LG-cs.CY-econ.GN`会给出类别属于cs.AI或cs.LG、但不属于cs.CY和econ.GN的论文列表；
- **arXiv类别差集入口1**：点击首页arXiv标题右上角的“+”号，可以选择自己感兴趣的类别，现在复选框改为三选款，留空为不选择，打勾为选择，减号为排除（即作差的类别），设置完成后回到首页，点击“➚”按钮即可访问相应的并差集链接；
- **arXiv类别差集入口2**：arXiv列表页的每一篇论文的Subjects行，悬停到某个subject后右侧会有“+”和“-”两个按钮，点击“+”则添加到关注类别，点击“-”则添加到排除列表，然后回到首页点击“➚”按钮即可访问相应的并差集链接。

### 2024.11.08
- **搜索页面增加订阅链接**：现在每个列页面顶部都增加了一个订阅链接，搜索页的订阅链接可以用来实现监控某些关键词的新论文的功能。

### 2024.11.01
- **默认隐藏部分作者列表**：当作者数超出40时，默认隐藏部分作者列表，点击“et al.”按钮才显示完整，以免作者列表篇幅过大影响阅读；
- **论文底部加上折叠按钮**：每篇论文加上折叠按钮，当点击了[PDF]或者[Kimi]时显示，用来缩回[PDF]和[Kimi]的输出内容。
...
### 2024.08.14
- **支持LaTeX渲染**：通过MathJax支持标题、摘要和Kimi-FAQ中的公式渲染，并且经过精心调试，能够较好地兼容网页翻译功能，不会因为启用翻译而导致公式乱码。
